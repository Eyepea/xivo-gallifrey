...
UPDATE `extensions`
SET `app` = 'Macro',
    `appdata` = CONCAT('agentdynamiclogin|${EXTEN:',LENGTH(`exten`),'}'),
    `exten` = CONCAT('_',`exten`,'.')
WHERE `name` = 'agentdynamiclogin' AND `app` = 'AgentLogin';

UPDATE `extensions`
SET `appdata` = CONCAT('agentstaticlogoff|${EXTEN:',LENGTH(`exten`),'}'),
    `exten` = CONCAT('_',`exten`,'.')
...
UPDATE `extensions`
SET `appdata` = CONCAT(`appdata`,'%${CONTEXT}@PICKUPMARK')
WHERE `name` = 'pickup' AND `appdata` NOT LIKE '%${CONTEXT}@PICKUPMARK';

UPDATE `extensions`
SET `appdata` = CONCAT(`appdata`,'|')
