...
sqlite_migrate() {
	...
	# backuping
	cp -a "${ASTSQLITE_DB}" "${BACKUP_DIR}/${ASTSQLITE_DB_FILENAME}-1.1.21-to-1.1.22-`date +%Y%m%d%H%M%S`"
...
mysql_migrate() {
	...
	# backuping
	mysqldump --defaults-extra-file=/etc/mysql/debian.cnf ${DBNAME} > "${BACKUP_DIR}/${DBNAME}-mysql.dump-1.1.21-to-1.1.22-`date +%Y%m%d%H%M%S`";
	if [ $? != 0 ]; then
