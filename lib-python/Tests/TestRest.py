...
class SelfXmlVisit(object):
	def fvisit(self, ctx, path, pos):
		return ctx.replace(
			cdt_classes_factory = lambda:((allow_xml,),),
			application_factory = lambda:self
		)

class XmlVisit(object):
	def fvisit(self, ctx, path, pos):
		return ctx.replace(cdt_classes_factory = lambda:((allow_xml,),))
...
class BoundedIfaceCls:
	...
	def get_tree_repr(self):
		...
		return r
	def req_in(self, ctx, adaptor, sa, payload_int):
		print adaptor.to_external(sa, (self.get_tree_repr(), self.iface.get_iface_name()))
...
class InterfacesCls(SelfXmlVisit, RT_Dyn):
	...
	def req_in(self, ctx, adaptor, sa, payload_int):
		...
		print adaptor.to_external(sa, (r, 'interfaces'))
