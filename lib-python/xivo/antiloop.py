...
def loop(graph, start):
	...
	Parameter graph must be a mapping object (in the Python sense) and have
...
def potential_by_card(graph):
	...
	with each vertex of the graph. The potential is the number of node that
	are adjacent to the considered one. """
...
def partial_order_from_reversed_ord_pot(rev_graph, v_pot):
	...
	of edges from the original graph. Edges that are in a loop are preferred
	for removal when they have the smallest possible "tension", where
	tension is defined by the potential difference between the destination
	and the source of the edge. When multiple possibilities exist because
	the same tension is present multiple times, the choice of an edge for
	removal shall be considered as an arbitrary one.
