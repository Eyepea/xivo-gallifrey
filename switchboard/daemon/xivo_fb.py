...
def manage_login(cfg, requester_ip, requester_port, socket):
        ...
        if int(version) < REQUIRED_CLIENT_VERSION:
                return commandclass.loginko_srv2clt(cfg, 'version_client:%s;%d' % (version, REQUIRED_CLIENT_VERSION))
        ...
        try:
                ...
                if userinfo == None:
                        ...
                        repstr = commandclass.loginko_srv2clt(cfg, 'login_passwd')
                else:
                        ...
                        if reterror is None:
                                ...
                                repstr = commandclass.loginok_srv2clt(userinfo, capa_user, [XIVOVERSION, __revision__], configs, cfg)
                ...
                if usefulmsg == "show_infos":
                    ...
                    if allow_events: # i.e. if SB-style connection
                        ...
                        if command.name in commandclass.get_list_commands_clt2srv():
                                ...
                                connid[0].send('Unknown Command %s\n' % command.name)
...
def handle_ami_event(astid, idata):
        ...
        for evt in evlist:
                ...
                if evfunction == 'Dial':
                        ...
                        try:
                                commandclass.dial(astid, this_event)
                        ...
                        if len(configs[astid].linkestablished) > 0:
                                try:
                                        user = '%s%s' % (dst.split('/')[0].lower(),
                                                         dst.split('/')[1].split('-')[0])
                                        userinfo = ulist[astid].finduser(user)
                        ...
                        try:
                                commandclass.link(astid, this_event)
                                
                                #plist_thisast.handle_ami_event_link(src, dst, clid1, clid2)
                        except Exception, exc:
                                log_debug(SYSLOG_ERR, '--- exception --- AMI / Link : %s (%s)' % (str(exc), str(this_event)))
                elif evfunction == 'Unlink':
                        try:
                                commandclass.unlink(astid, this_event)
                                ...
                                log_debug(SYSLOG_ERR, '--- exception --- AMI / Unlink : %s (%s)' % (str(exc), str(this_event)))
                elif evfunction == 'Hangup':
                        try:
                                commandclass.hangup(astid, this_event)

                                #chan  = this_event.get("Channel")
                                #cause = this_event.get("Cause-txt")
                                #plist_thisast.handle_ami_event_hangup(chan, cause)
                        except Exception, exc:
                                log_debug(SYSLOG_ERR, '--- exception --- AMI / Hangup : %s (%s)' % (str(exc), str(this_event)))
                        ...
                        commandclass.parkedcall(astid, this_event)
                        ...
                        cfrom   = this_event.get('From')
                        exten   = this_event.get('Exten')
                        commandclass.unparkedcall(astid, this_event)
                        ...
                        channel = this_event.get('Channel')
                        exten   = this_event.get('Exten')
                        commandclass.parkedcallgiveup(astid, this_event)
                ...
                elif evfunction == 'UserEventUserFeature':
                        pass
                        ...
                        print 'QueueMemberStatus', qname, location, status
...
while True: # loops over the reloads
        ...
        for astid in xivoconf.sections():
                if astid != 'general' and astid in asterisklist:
                        ...
                        commandclass.set_cdr_uri(cdr_db_uri)
        ...
        Fsock.bind(("", port_elect))
        Fsock.listen(10)
        ins.append(Fsock)

        ins.append(OperatSock)
        ...
        while not askedtoquit:
                ...
                if i:
                        ...
                        # the new UI (WEBI) connections are catched here
                                ...
                                log_debug(SYSLOG_INFO, "TCP (F) socket opened on   %s:%d" % (Fsockparams[0], Fsockparams[1]))
                                # appending the opened socket to the ones watched
                                ins.append(conn)
                                conn.setblocking(0)
                                tcpopens_fc.append([conn, Fsockparams[0], Fsockparams[1]])
                        ...
                        elif filter(lambda j: j[0] in i, tcpopens_fc):
                                conn = filter(lambda j: j[0] in i, tcpopens_fc)[0]
                                try:
                                        manage_tcp(conn)
                                except Exception, exc:
