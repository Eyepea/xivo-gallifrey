	...
	case 'add':
		...
		&& dwho_issa('phonehints',$_QR) === true)
		{
			...
			|| $app->add() === false)
			{
				...
				$_QRY->go($_TPL->url('service/ipbx/cti_settings/phonehints'),$param);
		}
...
$menu->set_toolbar('toolbar/service/ipbx/'.$ipbx->get_name().'/cti_settings/phonehints');

$_TPL->set_bloc('main','service/ipbx/'.$ipbx->get_name().'/cti_settings/phonehints/'.$act);
