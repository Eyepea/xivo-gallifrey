...
sqlite_migrate() {
	...
	# backuping
	cp -a "${XIVOSQLITE_DB}" "${BACKUP_DIR}/${XIVOSQLITE_DB_FILENAME}-pre-1.1.7-`date +%Y%m%d%H%M%S`"
...
mysql_migrate() {
	...
	# backuping
	mysqldump --defaults-extra-file=/etc/mysql/debian.cnf ${DBNAME} > "${BACKUP_DIR}/${DBNAME}-mysql.dump-pre-1.1.7-`date +%Y%m%d%H%M%S`";
	if [ $? != 0 ]; then
