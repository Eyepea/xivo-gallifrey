<?php
	...
	echo $url->href_html($url->img_html('img/menu/top/toolbar/bt-add.gif',$this->bbf('toolbar_opt_add'),'border="0"'),'xivo/configuration/servers','act=add',null,$this->bbf('toolbar_opt_add'));
...
<div class="sb-advanced-menu">
	<ul id="advanced-menu" onmouseover="this.style.display = 'block';" onmouseout="this.style.display = 'none';">	
		...
		<li><a href="#" onclick="this.tmp = xivo_fm['fm-servers-list']['act'].value; xivo_fm['fm-servers-list']['act'].value = 'deletes'; return(confirm('<?=xivo_stript($this->bbf('toolbar_adv_menu_delete_confirm'));?>') ? xivo_fm['fm-servers-list'].submit() : xivo_fm['fm-servers-list']['act'] = this.tmp);"><?=$this->bbf('toolbar_adv_menu_delete');?></a></li>
	</ul>
