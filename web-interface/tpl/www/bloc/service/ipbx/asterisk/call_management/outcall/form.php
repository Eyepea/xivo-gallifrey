<?php
	...
	if(($nnumlen = xivo_uint($info['outcall']['numlen'])) !== 0):
		$numlen = $nnumlen;
	else:
		$numlen = $info['outcall']['numlen'];
	endif;
?>
...
<?=$form->select(array('desc' => $this->bbf('fm_outcall_stripnum'),'name' => 'outcall[stripnum]','labelid' => 'outcall-stripnum','key' => false,'default' => $element['outcall']['stripnum']['default'],'value' => xivo_cast_except($info['outcall']['stripnum'],null,'uint')),$element['outcall']['stripnum']['value']);?>
...
<?=$form->select(array('desc' => $this->bbf('fm_outcall_weight'),'name' => 'outcall[weight]','labelid' => 'outcall-weight','key' => false,'default' => $element['outcall']['weight']['default'],'value' => xivo_cast_except($info['outcall']['weight'],null,'uint')),$element['outcall']['weight']['value']);?>
...
<?=$form->select(array('desc' => $this->bbf('fm_outcall_hangupringtime'),'name' => 'outcall[hangupringtime]','labelid' => 'outcall-hangupringtime','bbf' => array('mixkey','fm_outcall_hangupringtime-opt','paramarray'),'default' => $element['outcall']['hangupringtime']['default'],'value' => xivo_cast_except($info['outcall']['hangupringtime'],null,'uint')),$element['outcall']['hangupringtime']['value']);?>
