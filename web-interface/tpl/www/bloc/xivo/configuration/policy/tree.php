	...
	if(($arr = xivo_get_aks($tree)) !== false):
		...
		for($i = 0;$i < $arr['cnt'];$i++):
			...
			if($v['level'] === 3):
				...
				echo '<div class="ply-func">',$form->checkbox(array('desc' => array('%s%s',$this->bbf('ply_'.$v['id']),1),'name' => 'tree[]','label' => 'lb-'.$v['id'],'id' => $v['id'],'field' => false,'value' => $v['path'],'checked' => $v['access']),'onclick="xivo_fm_mk_policy(this);"'),'</div>',"\n";
			...
			if(isset($v['child']) === true):
				...
				$this->file_include('bloc/xivo/configuration/policy/tree',array('tree' => $v['child'],'parent' => $parent));
			endif;
			if($v['level'] === 3):
				echo '</div>';
			endif;
